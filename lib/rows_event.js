  ...
  if(config.hasOwnProperty(tableMap.tableName) && config[tableMap.tableName].type === 'node') {
    this.rows.forEach(function(rawRow) {
      if(rawRow.hasOwnProperty('before') && rawRow.hasOwnProperty('after')) {
        ...
        var originalKP = rawRow.before[config[tableMap.tableName].primaryKey];
      } else {
        var row = rawRow;
        var originalKP = rawRow[config[tableMap.tableName].primaryKey];
      }
      props = '';
      relations = [];
      console.log('--');
      Object.keys(row).forEach(function(name) {
        ...
        if(typeof config[tableMap.tableName].relations !== 'undefined' && config[tableMap.tableName].relations.hasOwnProperty(name) && row[name] !== null) {
          var relQuery = 'MATCH (a { mysql_id:\'' + originalKP + '\'}), (b { mysql_id: \'' + row[name] + '\'}) CREATE (a)-[:'+config[tableMap.tableName].relations[name]+']->(b)';
          relations.push(relQuery);
        }
      });

      var query = 'MERGE (instance:' + config[tableMap.tableName].name + ' { mysql_id: \'' + originalKP + '\'})' +
            ' ON CREATE SET ' + props +
            ' ON MATCH SET ' + props +
            ' RETURN instance'; 
      ...
      // neo4j.cypher({ query: query, params: {}}, function(err, res) {
      //   if (err) {
      ...
      //   }
      //   relations.forEach(function(rel) {
      //     neo4j.cypher({ query: rel, params: {}}, function(err, res) {
      //       if (err) {
    ...
    this.rows.forEach(function(row) {
      ...
      // neo4j.cypher({ query: relQuery, params: {}}, function(err, res) {
      //   if (err) {
