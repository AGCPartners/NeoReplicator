    ...
    this.rows.forEach(function(row) {
      ...
      Object.keys(row).forEach(function(name) {
        ...
        if(typeof config[tableMap.tableName].relations !== 'undefined' && config[tableMap.tableName].relations.hasOwnProperty(name)) {
          var relQuery = 'MATCH (a { mysql_id:\'' + row[config[tableMap.tableName].primaryKey] + '\'}), (b { mysql_id: \'' + row[name] + '\'}) CREATE (a)-[:'+config[tableMap.tableName].relations[name]+']->(b)';
          relations.push(relQuery);
        }
      });

      var query = 'MERGE (instance:' + config[tableMap.tableName].name + ' { mysql_id: \'' + row[config[tableMap.tableName].primaryKey] + '\'})' +
            ' ON CREATE SET ' + props +
            ' ON MATCH SET ' + props +
            ' RETURN instance'; 
